...
#  ifndef _WIN32_WINNT
#    define _WIN32_WINNT 0x0A00 // Windows 10 SDK, needed for symlink calls
#  endif
#else
...
bool SystemTools::FileIsSymlink(const std::string& name)
{
  ...
  if (!FindFirstFileW(Encoding::ToWindowsExtendedPath(name).c_str(),
                      &findData)) {
    return false;
  }
...
{
  // note: we want to use the flag SYMBOLIC_LINK_FLAG_ALLOW_UNPRIVILEGED_CREATE
  // here, but this constant is missing from all but the most recent Windows
  // SDKs and MinGW versions. Luckily, it is defined to have the fixed value 2,
  // so we can just use that. This way the code can be built with any SDK (but
  // will only work with Windows 10 >= 1607)
  DWORD flags = 0x02;
  ...
  // note: this call requires Developer Mode to be enabled in Settings on the
  // machine
  return CreateSymbolicLinkW(Encoding::ToWindowsExtendedPath(newName).c_str(),
                             Encoding::ToWide(origName).c_str(), flags);
}
#else
bool SystemTools::CreateSymlink(const std::string& origName,
                                const std::string& newName)
...
#if defined(_WIN32) && !defined(__CYGWIN__)
bool SystemTools::ReadSymlink(const std::string& symlink, std::string& target)
{
  if (!FileIsSymlink(symlink)) {
    return false;
  }

  // open the file so we can get its path
  HANDLE fileHandle =
    CreateFileW(Encoding::ToWindowsExtendedPath(symlink).c_str(), GENERIC_READ,
  ...
  // first try with no buffer to find correct buffer size
  DWORD bufferSize =
    GetFinalPathNameByHandleW(fileHandle, nullptr, 0, FILE_NAME_NORMALIZED);
  if (bufferSize == 0) {
    return false;
  }

  // now, allocate buffer and try for real
  wchar_t* pathBuffer = new wchar_t[bufferSize];

  if (!GetFinalPathNameByHandleW(fileHandle, pathBuffer, bufferSize,
  ...
  target = Encoding::ToNarrow(pathBuffer);
  ConvertToUnixSlashes(target);

  // erase the "//?/" at the beginning of the path if it exists
  if (target.length() >= 4 && target.substr(0, 4) == "//?/") {
    target.erase(0, 4);
  }
