 ...
 *********************************************************************/
// $Id: Rocmop_1.C,v 1.8 2008/12/06 08:45:24 mtcampbe Exp $
...
void Rocmop::smooth_in_place(COM::Attribute *pmesh){
  ...
  print_legible(1,"Exiting rocmop::smooth_in_place");
#ifdef ROCPROF
  Profile_end("Rocmop::smooth_in_place");
#endif
}
...
void Rocmop::invert_tets(){
#ifdef ROCPROF
  Profile_begin("Rocmop::invert_tets");
#endif
  print_legible(1,"Entering Rocmop::invert_tets");
  std::vector<Pane*> allpanes;
  _buf_window->panes(allpanes);
  for(int i=0, ni = allpanes.size(); i<ni; ++i){
  ...
  _invert = 0;
  print_legible(1,"Exiting Rocmop::invert_tets");
#ifdef ROCPROF
